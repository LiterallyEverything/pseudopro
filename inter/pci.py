import inter.builtinFunction as bif
from datetime import datetime, date
import re
import copy
from inter.settings import *
...
class error:
    ...
    def _raise(self, title, lineNo, line=None, pos=-1, description=None):
    ...
    def nameErr(self, lineNo, line, pos, detail=None, description=None):
        self._raise(
            f"Name error: <{detail}> referenced before assignment",
            lineNo,
            line,
            pos,
            description
    ...
    def nameConErr(self, lineNo, line, pos, detail=None, description=None):
        self._raise(
            f"Name error: <{detail}> conflicts with {description}",
            lineNo,
            line,
            pos
        )
    ...
    def typeErr(self, lineNo, line, pos, detail=None, description=None):
        desc = None
        if description:
            desc = f"{description} expected"

        self._raise(
            f"Type error: unexpected type <{detail}>",
            lineNo,
            line,
            pos,
            desc
    ...
    def indexErr(self, lineNo, line, pos, detail=None, description=None):
        self._raise(
            "Index error: array index out of range",
            lineNo,
            line,
            pos,
            description
    ...
    def argErr(self, lineNo, line, pos, detail=None, description=None):
        ...
        self._raise(
            f"Argument error: expected {exp} argument(s), got {got}",
            lineNo,
            line,
            pos
        )
    ...
    def valErr(self, lineNo, line, pos, detail=None, description=None):
        self._raise(
            f"Value error: '{detail}' does not match format '{description}'",
            lineNo,
            line,
            pos
        )
    ...
    def attrErr(self, lineNo, line, pos, detail=None, description=None):
        self._raise(
            f"Attribute error: <{detail}> has no attribute <{description}>",
            lineNo,
            line,
            pos
        )
    ...
    def attrUnAss(self, lineNo, line, pos, detail=None, description=None):
        self._raise(
            f"Attribute error: <{detail}>'s <{description}> is not mutable",
            lineNo,
            line,
            pos
        )
...
class interpreter:
    ...
    def isObject(self, identifier, lineNo, line):
        if "." in identifier:
            obj = identifier[:identifier.find(".")].strip()
...
##### Utility functions #####
    ...
    def getValue(self, identifier, lineNo, line, ignoreDate = True):
        ...
        elif re.fullmatch(r"-?\d+\.\d+", identifier):
...
class procedure(function):
    def __init__(self, identifier, type, initialpos, lines, byref, attributes, parameters={}):
        super().__init__(identifier, type, initialpos, lines, attributes, parameters)
        self.byref = byref
